...
function findClasses(cssText) {
  // Match only class selectors that start with a letter, underscore or hyphen
  // and appear in selector contexts (preceded by start, whitespace, comma or brace).
  const classRegex = /(^|\s|,|\})\.([A-Za-z_-][A-Za-z0-9_-]*)/gm;
  const classes = new Set();
  let m;
  while ((m = classRegex.exec(cssText)) !== null) classes.add(m[2]);
